...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    messageData = _inputTextField.text;
...
- (IBAction)sendFile:(id)sender {
    ...
    // prepare audio message using GGWave
    {
        ...
        NSString *filepathJSON = [[NSBundle mainBundle] pathForResource:@"BLEServices" ofType:@"json"];
        NSString *filepathUPG = [[NSBundle mainBundle] pathForResource:@"342" ofType:@"upg"];
        NSData *fileData = [NSData dataWithContentsOfFile:filepathJSON];//filepathJSON changge for json
        NSString *res = [[NSString alloc] initWithData:fileData encoding:NSUTF8StringEncoding];
        _labelReceived.text = [@"Received: " stringByAppendingString:[NSString stringWithFormat:@"%@", res]];

         char *byteUPG = (char *)[[NSMutableData dataWithData:fileData] bytes];

        const int len1 = (int) strlen(byteUPG);

       // const int n = ggwave_encode(stateOut.ggwaveId, payload, len, GGWAVE_TX_PROTOCOL_AUDIBLE_FAST, 10, NULL, 1);
        const int n = ggwave_encode(stateOut.ggwaveId, byteUPG, len1, GGWAVE_TX_PROTOCOL_AUDIBLE_FAST, 10, NULL, 1);

       // stateOut.waveform = [NSMutableData dataWithLength:sizeof(char)*n];
        stateOut.waveform = [NSMutableData dataWithLength:sizeof(char)*n];

        const int ret = ggwave_encode(stateOut.ggwaveId, byteUPG, len1, GGWAVE_TX_PROTOCOL_AUDIBLE_FAST, 10, [stateOut.waveform mutableBytes], 0);
...
- (IBAction)togglePlayback:(id)sender {
    ...
    // prepare audio message using GGWave
    {
        const char * payload = [_inputTextField.text UTF8String];
